        ...
        if (data.status === "OK") {
          ...
          if (solved) {
            ...
            if (isLast) {
              ...
              // getWinner will be computed on the server technically, but we can do a dummy finish update
              // the server GET handles time checking, but we can fast-track finish
            }
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white font-sans flex flex-col relative overflow-hidden">
      ...
      {room.status === "finished" && (
        <div className="absolute inset-0 bg-neutral-950/80 backdrop-blur-2xl z-40 flex items-center justify-center p-4">
          <div className="text-center bg-white/5 border border-white/10 p-12 rounded-3xl shadow-2xl max-w-lg w-full transform animate-in fade-in zoom-in duration-500">
            ...
            <div className="py-4 border-t border-white/10">
              ...
              <p className="text-3xl font-extrabold text-yellow-400 mt-1">{room.winner || "Draw"}</p>
            </div>
          </div>
        </div>
      )}
